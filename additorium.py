...
def train_model(values):
    ...
    average_output = [0.5,0.5,0.5,0.5,0.5,0.5,0.5]
    #Iterate
    for iteration in range(iterations):
        ...
        for k in range(len(weights_classes)):

            random_training_vector = []
            while True:
                if average_output[k] > 0.01 and average_output[k] < 0.99:
                    ...
                    if random_training_vector[-1] == weights_classes[k] and iteration % 2 == 0:
                        break
                    elif random_training_vector[-1] != weights_classes[k] and iteration % 2 != 0:
                        break
                    else:
                        pass
                #If the average output is very low it's training with too many non-matches. Get a match instead
                elif average_output[k] <= 0.01:
                    #print("Match vector")
                    random_training_index = int(len(all_training_data)*random.random())
                    random_training_vector = all_training_data[random_training_index]
                    if random_training_vector[-1] == weights_classes[k]:
                        break
                    else:
                        pass
                #If the average output is very high it's training with too many matches. Get a non-match instead.
                elif average_output[k] >= 0.99:
                    #print("non-match vector")
                    random_training_index = int(len(all_training_data)*random.random())
                    random_training_vector = all_training_data[random_training_index]
                    if random_training_vector[-1] != weights_classes[k]:
                        break
                    ...
                    print("Error: average output range not found.")
            ...
            weights_class = weights_classes[k]
            target = dec(0.0000)
            ...
            for i in range(len(all_weights_tup)):
                ...
                if current_error > 0.0025:
                    ...
                    all_weights = update_weights_iterator(all_weights, derror_dweights, learning_rate, i)
        ...
        if iteration % error_sampling_rate == 0 and iteration > 2:
            error_sample, average_output = get_error_sample(weights_classes, error_sampling_size, all_training_data)
...
def get_error_sample(weights_classes, error_sampling_size, all_training_data):
    connection = db.create_connection("additorium.db")
    cumulative_error = 0
    average_output = []
    for k in range(len(weights_classes)):
        ...
        for i in range(error_sampling_size):
            ...
            while True:
                random_training_index = int(len(all_training_data)*random.random())
                random_training_vector = all_training_data[random_training_index]
                if random_training_vector[-1] == weights_classes[k] and i % 2 == 0:
                    break
                elif random_training_vector[-1] != weights_classes[k] and i % 2 != 0:
                    break
                else:
                    pass
            weights_class = all_weights[-1][-1]
            target = dec(0.0000)
            if random_training_vector[-1] == weights_class:
                target = dec(1.00000)
    ...
    cumulative_error = cumulative_error / (error_sampling_size*len(weights_classes))
    #print(average_output)
    return cumulative_error, average_output

def update_weights_iterator(all_weights_tup, derror_dweights, learning_rate, i):
    ...
    for j in range(len(all_weights[i])):
        if j == 17 or j == 18:
            ...
            if j == 16:
                all_weights[i][j] = dec(all_weights[i][j]) - dec(derror_dweights[i][j])*dec(learning_rate)*1 
            else:
                all_weights[i][j] = dec(all_weights[i][j]) - dec(derror_dweights[i][j])*dec(learning_rate)
...
def compute_layer_1(weights,vector):
    output = []
    for i in range(10):
        this_output = (dec(weights[i]))*(dec(vector[i])**5)
        output.append(this_output)
    return output

def compute_layer_2(weights,layer1):
    output = []
    for i in range(10):
        output.append(dec(weights[i])*(dec(layer1[i])**4))
    return output

def compute_layer_3(weights,layer2):
    output = []
    for i in range(10):
        output.append(dec(weights[i])*(dec(layer2[i])**3))
    return output

def compute_layer_4(weights,vector):
    #print(f"Weights[0]: {weights[0]}")
    #print(f"Weights[1]: {weights[1]}")
    #print(f"Weights[2]: {weights[2]}")
    #print(f"Weights[3]: {weights[3]}")
    #print(f"Vector: {vector}")
    output = []
    for i in range(len(vector)-1):
        #print(f"(weights[0][i]): {(weights[0][i])}")
        #print(f"(vector[i]): {(vector[i])}")
        output.append(f"{dec(weights[0][i])*(dec(vector[i])*dec(vector[i]))}")
    #print(f"layer 4 output: {output}")
    return output
...
def compute_layer_5(weights, vector, layer4):
    ...
    output = []
    for i in range(len(vector)-1):
        output.append(f"{dec(weights[1][i])*(dec(vector[i]))+dec(layer4[i])}")
...
def compute_layer_6(weights, vector, layer5):
    output = []
    for i in range(len(vector)-1):
        output.append(f"{dec(weights[2][i])+dec(layer5[i])}")
    #print(f"layer 6 output: {output}")
    return output

def compute_layer_7(weights,layer6):
    output = dec(0.0000000000000000000000000)
    #print(len(weights))
    for i in range(len(weights[3])-3):
        #print(weights[i])
        output = output + (dec(weights[3][i])*(dec(layer6[i])))
        #print(f"{weights[3][i]}; {layer6[i]}")
    #print(f"layer 7 dot product: {output}")
    output = output + dec(weights[3][-3]) #add bias
...
def evaluate_model(values):
    ...
    for bean_class in bean_classes:
        ...
        ORDER BY id ASC;
        """
        these_weights = db.execute_read_query(connection,read_weights)
        all_weights.append(these_weights)
    percent_correct = []
    for index in range(len(bean_classes)):

        read_input_vectors = f"""
        SELECT Area, Perimeter, MajorAxisLength, MinorAxisLength, AspectRatio, Eccentricity, ConvexArea, EquivDiameter, Extent, Solidity, Roundness, Compactness, ShapeFactor1, ShapeFactor2, ShapeFactor3, ShapeFactor4, Class 
        FROM training_data
        WHERE class = "{bean_classes[index]}";
        """
        ...
        for vector in these_vectors:
            ...
            #print(maximum_index)
            if bean_classes[maximum_index] == vector[-1]:
                total_correct = total_correct +1
        percent = round((total_correct / len(these_vectors))*100,2)
        percent_correct.append(percent)
    window["-Evaluate_Messages-"].update(f"""
    {bean_classes[0]} had a {percent_correct[0]}% success rate.
    {bean_classes[1]} had a {percent_correct[1]}% success rate.
    {bean_classes[2]} had a {percent_correct[2]}% success rate.
    {bean_classes[3]} had a {percent_correct[3]}% success rate.
    {bean_classes[4]} had a {percent_correct[4]}% success rate.
    {bean_classes[5]} had a {percent_correct[5]}% success rate.
    {bean_classes[6]} had a {percent_correct[6]}% success rate.
    """)
